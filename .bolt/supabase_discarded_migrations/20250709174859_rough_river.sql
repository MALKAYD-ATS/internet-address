/*
  # Fix Home Page Dynamic Content Tables
...
DO $$
BEGIN
  -- Enable RLS with checks
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'home_page_title_section' AND relrowsecurity = true) THEN
    ALTER TABLE home_page_title_section ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'home_numbers' AND relrowsecurity = true) THEN
    ALTER TABLE home_numbers ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'choose_ats' AND relrowsecurity = true) THEN
    ALTER TABLE choose_ats ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'drone_solutions' AND relrowsecurity = true) THEN
    ALTER TABLE drone_solutions ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'principles' AND relrowsecurity = true) THEN
    ALTER TABLE principles ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'heading' AND relrowsecurity = true) THEN
    ALTER TABLE heading ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'ventures' AND relrowsecurity = true) THEN
    ALTER TABLE ventures ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'contact_information' AND relrowsecurity = true) THEN
    ALTER TABLE contact_information ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'contact_locations' AND relrowsecurity = true) THEN
    ALTER TABLE contact_locations ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'student_success_stories' AND relrowsecurity = true) THEN
    ALTER TABLE student_success_stories ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'newsletter' AND relrowsecurity = true) THEN
