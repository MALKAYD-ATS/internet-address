...
const Training: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Regulation Courses Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {courses.filter(course => course.type === 'regulation').length > 0 && (
          <div className="mb-16">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-4">Regulation Courses</h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Essential regulatory training and certification courses for drone operations compliance.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {courses.filter(course => course.type === 'regulation').map((course) => {
        ...
        {/* Application Courses Section */}
        {courses.filter(course => course.type === 'application').length > 0 && (
          <div className="mb-16">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-4">Application Courses</h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Practical application courses focusing on real-world drone operations and specialized use cases.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {courses.filter(course => course.type === 'application').map((course) => {
            const isFlipped = flippedCards.has(course.id);
            const details = courseDetails[course.id];
            const isLoadingDetails = loadingDetails.has(course.id);
            const isRegistering = registering.has(course.id);
        ...
        {courses.length === 0 ? (
          ...
          /* Show message if no courses match the current filters */
          courses.filter(course => course.type === 'regulation').length === 0 && 
          courses.filter(course => course.type === 'application').length === 0 && (
            <div className="text-center py-12">
              <BookOpen className="w-16 h-16 text-gray-400 mx-auto mb-4" />
              <h3 className="text-lg font-medium text-gray-900 mb-2">No courses match the current criteria</h3>
              <p className="text-gray-500">All courses are currently being categorized.</p>
            </div>
          )
        )}
      </div>
